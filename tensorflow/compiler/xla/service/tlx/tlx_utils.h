...
llvm::Value* GetLayoutVector(const &Shape TensorShape, llvm::LLVMContext* C){
    ...
    std::vector<llvm::ConstantInt*> LayoutArray;



    llvm::Type* I32Ty = llvm::Type::getInt32Ty(*C);
    llvm::ConstantInt* Zero =  llvm::ConstantInt::get(I32Ty, 0)
    llvm::ConstantInt* One =  llvm::ConstantInt::get(I32Ty, 0)

    if(Layout.minor_to_major()){
    ...
    llvm::Constant* LayoutVector = llvm::ConstantVector::get(llvm::ArrayRef<Constant*>(LayoutArray));
...
llvm::CallInst* CreateTypeInfoCall(llvm::Value* Vector, llvm::Value* Shape, llvm::Value* Layout, llvm::Value* Padding, llvm::IRBuilder<>* b_){
    ...
    llvm::Function* TypeInfoFn = llvm::Intrinsic::getDeclaration(M, llvm::Instrinsic::tensor_typeinfo, llvm::ArrayRef<llvm::Type*>(TypeInfoArgsTy) );
