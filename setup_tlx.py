...
def init_tlx():
    ...
    execute(install_tlx_cmd, cwd=tlx_dir_root)

    print("*" * 5, "Successfully built TLX repository", "*" * 5)

    llvm_project_dir = os.path.join(tlx_dir_root, "llvm-project")

    llvm_project_dir = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), llvm_project_dir
    )
...
def extend_tlx_repo(tlx_dir_root):
    ...
    cp_cmd = [
        "cp",
        os.path.join("third_party", "llvm", "llvm.autogenerated.BUILD"),
    ...
    cp_cmd = [
        "cp",
        os.path.join("third_party", "mlir", "BUILD"),
    ...
    cp_cmd = [
        "cp",
        os.path.join("third_party", "mlir", "test.BUILD"),
...
def build_tensorflow(tlx_dir_root):
    ...
    tf_deps_cmd = " ".join(tf_deps_cmd)
    execute(tf_deps_cmd)

    print("Configuring Tensorflow ...")
    configure_tf_cmd = ["python", "configure.py"]
    configure_tf_cmd = " ".join(configure_tf_cmd)
    execute(configure_tf_cmd)
    ...
    pip_uninstall_cmd = ["pip3", "uninstall", "-y", "tensorflow"]
